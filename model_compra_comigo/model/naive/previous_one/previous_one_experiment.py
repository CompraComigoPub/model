"""
Implementation of a PreviousOne Naive experiment .
"""
from numpy import ndarray, array, concatenate, apply_along_axis, mean
...
from model_compra_comigo.model.naive.previous_one.previous_one_model import (
    PreviousOneModel,
)
...
class PreviousOneExperiment:
    """
    Implementation of a Experiment that used Naive Model Previous One .
    ...
    run(
        ...
        run_name: Optional[str] = "Previous One Model",
        experiment_id: Optional[str] = None,
    ) -> None
        PreviousOneExperiment run. Fits nothing, since PreviousOneModel does
    ...
    def __init__(
        ...
        """
        Constructor for PreviousOneExperiment .
    ...
    def run(
        ...
        run_name: Optional[str]
            Run name. Defaults to "Previous One Model" .
        ...
        with start_run(run_name=run_name, experiment_id=experiment_id) as run:
            ...
            if train_data:
                # Train dataset
                train_evaluation = PreviousOneExperiment.evaluate(
                    model=model,
            ...
            if validation_data:
                # Validation dataset
                validation_evaluation = PreviousOneExperiment.evaluate(
                    model=model,
                    data=windowed_validation_data,
                    window_size=window_size,
                    nforecast=nforecast,
                )
            test_evaluation = PreviousOneExperiment.evaluate(
                model=model,
            ...
            if generate_full_visualization:
                ...
                if train_data:
                    ...
                    PreviousOneExperiment.forecast_and_visualize(
                        model=model,
                        data=train_data,
                        window_size=window_size,
                        nforecast=nforecast,
    ...
    def forecast_and_visualize(
        ...
        PreviousOneExperiment.create_gif(
            time_data=data[0],
            series=data[1],
